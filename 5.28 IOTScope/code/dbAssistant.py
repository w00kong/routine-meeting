...
def rspsXML2dbs(vendor, model):
    ...
    id = 0
    conn = sqlite3.connect('{}_{}.db3'.format(vendor, model))
    cursor = conn.cursor()

    cursor.execute("DROP TABLE if exists unauth")
    cursor.execute("CREATE TABLE unauth (id INTEGER PRIMARY KEY AUTOINCREMENT, url VARCHAR(200) NOT NULL UNIQUE, statusCode VARCHAR(10) NOT NULL , content VARCHAR(200) NOT NULL );")
    cursor.execute("DROP TABLE if exists auth")
    cursor.execute("CREATE TABLE auth (id INTEGER PRIMARY KEY AUTOINCREMENT, url VARCHAR(200) NOT NULL UNIQUE, statusCode VARCHAR(10) NOT NULL , content VARCHAR(200) NOT NULL );")
    cursor.execute("DROP TABLE if exists potential")
    cursor.execute("CREATE TABLE potential (id INTEGER PRIMARY KEY AUTOINCREMENT, url VARCHAR(200) NOT NULL UNIQUE, statusCode VARCHAR(10) NOT NULL , content VARCHAR(200) NOT NULL );")
    conn.commit()
...
def content2html(vendor, model, url, firmadyne=False):
    if firmadyne:
        conn = sqlite3.connect('{}_{}_firmadyne.db3'.format(vendor, model))
    else:
        conn = sqlite3.connect('{}_{}.db3'.format(vendor, model))
    cursor = conn.cursor()
...
def createDBauthDiff(vendor, model, firmadyne):
    if firmadyne:
        dbFile = "{}_{}_firmadyne.db3".format(vendor, model)
    else:
        dbFile = "{}_{}.db3".format(vendor, model)
    ...
    cursor.execute("CREATE TABLE authDiff (id INTEGER PRIMARY KEY AUTOINCREMENT, url VARCHAR(200) NOT NULL UNIQUE, statusCode VARCHAR(10) NOT NULL , content VARCHAR(200) NOT NULL );")
    conn.commit()
...
def createDBexist(vendor, model, firmadyne):
    if firmadyne:
        ...
        uaiFile = './' + vendor + '/' + model + '/uai.txt'
        dbFile = "{}_{}.db3".format(vendor, model)
    #print (vendor, model)
    print ("createDBexist ->", uaiFile, dbFile)
    ...
    cursor.execute("CREATE TABLE potential_exist (id INTEGER PRIMARY KEY AUTOINCREMENT, url VARCHAR(200) NOT NULL UNIQUE, statusCode VARCHAR(10) NOT NULL , content VARCHAR(200) NOT NULL )")
    cursor.execute("DROP TABLE if exists authDiff_exist")
    cursor.execute("CREATE TABLE authDiff_exist (id INTEGER PRIMARY KEY AUTOINCREMENT, url VARCHAR(200) NOT NULL UNIQUE, statusCode VARCHAR(10) NOT NULL , content VARCHAR(200) NOT NULL )")
    conn.commit()
...
if __name__ == "__main__":
    ...
    ips = ['http://192.168.0.50',
           ...
           'http://192.168.1.1']
    '''
    pathFile = ['D-Link_DIR-816.txt']
    #pathFile = ['D-Link_DAP1320.txt']
    #pathFile = ['D-Link_DIR868L.txt']
    #pathFile = ['Netgear_WNDR4000.txt']
    #ips = ['http://192.168.0.50']
    ips = ['http://192.168.0.1']
    ...
    for vm in vms:
        ...
        createDBexist(vendor, model, firmadyne)
        checkDBExist(vendor, model, firmadyne)
