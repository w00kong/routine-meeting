...
def reqUrl(url, verbose=False):
    try:
        ...
        if (status1 == status2 and resp1 == resp2):
            ...
            #tmptime = random.randint(10, 15) + 3
            #time.sleep(tmptime)
...
if __name__ == "__main__":
    ...
    #urlDic = {"DAP1320": "192.168.0.50"}
    urlDic = {"DIR816": "192.168.0.1"}
    #urlDic = {"WNDR4000": "192.168.1.1"}
    ...
    for vm in vms:
        ...
        for path in paths:
            #print ('model =', model)
            #print ("urlDic[model] =", urlDic[model])
            #url = urljoin(urlDic[model], path)
            url = "http://" + urlDic[model] + path
            #print ('url =', url)
            tmpflag = reqUrl(url.strip())
            if tmpflag == False:
                if flag == False:
                    sys.exit(1)
                else:
